...
def uncertainty_to_string(x, err=None, precision=1):
    ...
    if err is None and hasattr(x, "__iter__"):
        x,err = x
    ...
    #TODO quickhack: when changing fit parameters from free to fixed on-the-fly,
    #                you don't want to encounter errors
    ...
    if err == 0:
        ...
        fieldw = x_exp - no_exp
        fmt = '%%.%df' % fieldw
        result1 = (fmt + "e%d") % (no_int*10**(-fieldw), x_exp)
        
        # format - nom(unc)
        fieldw = max(0, -no_exp)
        fmt = "%%.%df" % fieldw
        result2 = (fmt + "") % (no_int*10**no_exp)
...
if __name__ == "__main__":
    xs    = [0, 12.34567, -0.123456, 0.001234560000, -0.0000123456, float('nan'), 0, 123.456, 123.456, 12345.6]
    xes   = [   1e-4,  0.00123, 0.000123, 0.000000012345, 0.0000001234, 1, float('nan'), 0.1, 0, 0]
    precs = [     1,        2,        3,              4,            1, 1, 1, 1, 1, -3]
